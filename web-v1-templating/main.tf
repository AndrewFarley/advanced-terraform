...
# This is our init script but with a modification to also set the hostname to whatever we are going to set the instance name to 
data "template_file" "userdata" {
  template = "${file("../helpers/install-apache-set-hostname.sh")}"
  vars {
    terraform_hostname = "${var.env}-web-1"
  }
}
